...
Transform::Transform() { //En la clase transform tengo un contructor de nombre transform, asi se implementa 
...
void Transform::SetPosition(float x, float y, float z) {
	_position = glm::vec3(x, y, z);//Este cambia la posicion a la que nos digan
	UpdateModelMatrixPosition(); //metodo cool 
}
...
void Transform::UpdateModelMatrixPosition() { //calcula la rotacion, La escala "Esta en la diagonal" 
	_modelMatrix[3][0] = _position.x;
	_modelMatrix[3][1] = _position.y;
	_modelMatrix[3][2] = _position.z;
}

void Transform::UpdateModelMatrixRotationScale() { //se le agrega la escala a la matrix
	...
	UpdateModelMatrixPosition();
	UpdateLocalVectors(); //Actualizamos hacia donde apuntan mis vectores ahora
}

void Transform::UpdateLocalVectors() {
	glm::mat4 rotationMatrix = glm::mat4_cast(_rotation); //matriz de rotacion solo con rotacion, no escala no posicion. 
	_forward = glm::vec3(_modelMatrix * glm::vec4(WORLD_FORWARD_VECTOR, 0.0f));
	_up = glm::vec3(_modelMatrix * glm::vec4(WORLD_UP_VECTOR, 0.0f));
	_right = glm::cross(_up, _forward); //Aqui se multiplica el producto cruz de los dos anteriores 
}
